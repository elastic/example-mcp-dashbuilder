...
export function registerManageDashboard(server: McpServer): void {
  registerTool(server, 'create_dashboard', {
    ...
    const { id, dashboard } = createDashboard(args.title as string, args.id as string | undefined);
    return {
      content: [{
        type: 'text',
  ...
  registerTool(server, 'list_dashboards', {
    ...
    const list = dashboards
      .map((d) => `${d.isActive ? '→ ' : '  '}${d.id}: "${d.title}" (updated: ${d.updatedAt || 'never'})`)
      .join('\n');
    return { content: [{ type: 'text', text: `Dashboards:\n${list}` }] };
  });

  registerTool(server, 'switch_dashboard', {
    title: 'Switch Dashboard',
    description: 'Switch to a different dashboard by its ID. Use list_dashboards to see available IDs.',
    inputSchema: {
  ...
  registerTool(server, 'delete_dashboard', {
  ...
  registerTool(server, 'set_dashboard_title', {
  ...
  registerTool(server, 'remove_chart', {
    title: 'Remove Chart',
    ...
    return { content: [{ type: 'text', text: `Chart "${args.chartId}" removed. Dashboard now has ${dashboard.charts.length} chart(s).` }] };
  });

  registerTool(server, 'get_dashboard', {
    title: 'Get Dashboard',
  ...
  registerTool(server, 'clear_dashboard', {
