...
export const importFromKibanaTool = {
  name: 'import_from_kibana' as const,
  description:
  ...
  execute: async (args: { dashboardId: string; title?: string }) => {
    ...
    try {
      ...
      return {
        content: [
          {
            type: 'text' as const,
            text: `Failed to connect to Kibana at ${KIBANA_URL}: ${message}`,
          },
        ],
        isError: true,
      };
    ...
    if (!response.ok) {
      ...
      return {
        content: [
          { type: 'text' as const, text: `Kibana API returned ${response.status}: ${errorBody}` },
        ],
        isError: true,
      };
    }
    ...
    const savedObject = (await response.json()) as {
      attributes: { title: string; panelsJSON: string; sections?: KibanaSection[] };
    };
    ...
    try {
      ...
      return {
        content: [
          { type: 'text' as const, text: 'Failed to parse panelsJSON from Kibana dashboard.' },
        ],
        isError: true,
      };
    }
    ...
    for (const section of kibanaSections) {
      ...
      const panelIds = panels
        ...
        .map((p) => {
          const pAttrs = p.embeddableConfig?.attributes;
          const pTitle = String(pAttrs?.title) || p.panelIndex;
