...
export function registerPreview(server: McpServer): void {
  registerTool(server, 'preview', {
    ...
    return {
      content: [{ type: 'text', text: `Opening dashboard preview at ${PREVIEW_URL}. Make sure the preview app is running with "npm run dev:preview".` }],
    };
  });
}
