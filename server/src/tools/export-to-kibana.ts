...
export const exportToKibanaTool = {
  name: 'export_to_kibana' as const,
  description:
  ...
  execute: async (args: { title?: string }) => {
    ...
    if (dashboard.charts.length === 0) {
      return {
        content: [
          { type: 'text' as const, text: 'No charts to export. Create some charts first.' },
        ],
        isError: true,
      };
    }
    ...
    if (!dashboardId) {
      return `Dashboard created but could not extract ID: ${JSON.stringify(result)}`;
    }
